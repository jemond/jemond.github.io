...
Reference renders: \`Home — Final.dc.html\` and \`Essay — Final.dc.html\`.

The homepage and the essay page are one design: same 640px measure, same
190px margin column, same two rule weights, same three faces. On an essay the
margin column holds pull quotes; on the homepage it holds section labels.

**The homepage in \`index.html\` is not complete content.** It carries the real
bio, the current reading list, greatest hits, two suggested lists, and the
abandoned list, plus the first 8 book entries. Two \`⚠️ PORT\` comments mark
where the remaining 243 book entries and the remaining suggested lists go —
copy them from the live site into the same markup pattern.

## Files

| File | Goes to |
|---|---|
| \`site.css\` | \`/site.css\` (one stylesheet for the whole site) |
| \`index.html\` | \`/index.html\` — homepage, restyled to match |
